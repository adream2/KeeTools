...
| 8 | `CHANGELOG.md` | **网站**变更记录（框架 / 前台 / 后台 / 工程规范）；工具变更只记 `tools/{id}/CHANGELOG.md` |
...
## 十一、当前阶段
- **日常维护入口**：新增工具走 `scripts/new_tool.py` 脚手架 → 实现 → 六项门禁 + `check_tools_runtime.py` → CLI/后台扫描入库 → 落 `tools/{id}/CHANGELOG.md`（工具变更不进根 `CHANGELOG.md`）；改动前通读本文件与 `docs/` 规范
